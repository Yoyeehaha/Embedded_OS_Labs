...
static uint8_t ucLEDSuspended = 0;

void vButtonTask(void *pvParameters){
    while(1) {
        if (HAL_GPIO_ReadPin(BLUE_BUTTON_GPIO_Port, BLUE_BUTTON_Pin)) {
            //To avoid button bouncing problem
            ...
            if (!longPressTriggered && ucLEDSuspended == 0) {
                int toggle = 1;
                xQueueSend(MsgQueue, &toggle, 0);
            }
        }
        vTaskDelay(pdMS_TO_TICKS(10));
    }
...
void vLEDTask(void *pvParameters) {
    int msg = 0; //To inform mode swap, if msg == 1, need to change mode
    int mode = 0; //To know which mode is it now

    while(1) {
        if (mode == 0) {
            const uint16_t leds[] = {RED_LED_Pin, ORANGE_LED_Pin, GREEN_LED_Pin};
            for (int i = 0; i < 3; i++) {
                HAL_GPIO_WritePin(GPIOD, RED_LED_Pin|GREEN_LED_Pin|ORANGE_LED_Pin, GPIO_PIN_RESET);
                HAL_GPIO_WritePin(GPIOD, leds[i], GPIO_PIN_SET);

                //represent that queue has get something in 1000ms
                if (xQueueReceive(MsgQueue, &msg, pdMS_TO_TICKS(1000))) {
                    mode = 1;
                    break;
                }
            }
            ...
            if (xQueueReceive(MsgQueue, &msg, pdMS_TO_TICKS(500))) {
                mode = 0;
                continue;
            }
...
int main(void)
{
  ...
  MX_GPIO_Init();
  /* USER CODE BEGIN 2 */
  MsgQueue = xQueueCreate(1, sizeof(int));

  xTaskCreate(vLEDTask, "LED Task", 128, NULL, 1, &xLEDHandle);
