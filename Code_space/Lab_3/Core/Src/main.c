...
#define MONITOR_SENSOR_WAKEUP                MEMS_Write(0x21, 0x01); \
...
SemaphoreHandle_t xGreenPermit = NULL; // for green LED permission
/* USER CODE END PV */
...
void Green_LED_Task(void *pvParameters){
	for(;;){
		...
		xSemaphoreTake(xGreenPermit, 0xFFFF);
		HAL_GPIO_TogglePin(LED_Green_GPIO_Port, LED_Green_Pin);
		xSemaphoreGive(xGreenPermit);
...
void vHandlerTask( void *pvParameters ){
	...
	for(;;){
		...
		if (xSemaphoreTake(xSemaphore, 0xFFFF) == pdTRUE) {
			// Take the semaphore of Green LED blinking permission
			xSemaphoreTake(xGreenPermit, 0xFFFF);
			...
			// Give it back to Green LED
			xSemaphoreGive(xGreenPermit);
		}

	    // reset interrupt register
		MONITOR_SENSOR_WAKEUP
...
int main(void)
{
  ...
  xGreenPermit = xSemaphoreCreateBinary();
  // Let the Green LED blink first
  xSemaphoreGive(xGreenPermit);

  //task create
  xTaskCreate(Green_LED_Task, "Green LED", 128, NULL, 1, NULL);
  xTaskCreate(vHandlerTask, "Task Handler", 128, NULL, 2, NULL);
  vTaskStartScheduler();
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOD, LED_Green_Pin|LED_Orange_Pin|LED_Red_Pin|LD6_Pin
  ...
  /*Configure GPIO pins : LED_Green_Pin LED_Orange_Pin LED_Red_Pin LD6_Pin
                           Audio_RST_Pin */
  GPIO_InitStruct.Pin = LED_Green_Pin|LED_Orange_Pin|LED_Red_Pin|LD6_Pin
                          |Audio_RST_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
